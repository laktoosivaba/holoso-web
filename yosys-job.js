...
// `<top>.v` is read first, then the support-library closure (holoso_support.v) -- read_verilog accepts the
// whole bag in one invocation, so module resolution is order-independent. The `-I.` lets `<top>.v` resolve its
// `include "holoso_support.vh"` from the job dir, where the worker drops it. hierarchy -check aborts on any
...
// synthScript (top + holoso_support.v closure together; module resolution order-independent).
