...
//
// Flow per request: take the synthesized module + its holoso_support.{v,vh} (from the synth result),
// compute the instantiation closure (closure.js), run read_verilog -> hierarchy -check -> synth[/_arch] ->
// write_json, and report the post-synth cell-type histogram parsed from the JSON netlist. holoso_support.v is
// a single self-contained library that inlines every primitive the generated module instantiates (the wrappers
// plus the vendored float cores), so it is the only .v the closure ever needs; holoso_support.vh holds the
// function header that the generated module `include`s, so it must sit in the FS for read_verilog to resolve.
...
// Re-emit a @yowasp fetch-progress event to the UI as a download fraction (shown while a tool's WASM
// resources stream in on first use).
const reportFetch = (e) => {
  if (e.totalLength) postMessage({ type: "progress", done: e.doneLength, total: e.totalLength });
};

async function estimate(req) {
  const { top, verilog, support, supportVh, target } = req;
  ...
  let outputs;
  try {
    outputs = await runYosys(["job.ys"], files, { stdout: sink, stderr: sink, decodeASCII: true, fetchProgress: reportFetch });
  } catch (err) {
...
async function route(req) {
  const { top, verilog, support, supportVh, device, pkg } = req;
  ...
  let pnrOut;
  try {
    pnrOut = await runNextpnr(nextpnrArgs({ device, pkg }), { "design.json": designJson }, { stdout: pnrSink, stderr: pnrSink, decodeASCII: true, fetchProgress: reportFetch });
  } catch (err) {
...
function warmUp() {
  ...
  return runYosys(["-V"], {}, { stdout: () => {}, stderr: () => {}, fetchProgress: reportFetch }).then(() => postMessage({ type: "ready" }));
}
