...
class _StreamRedirect(io.TextIOBase):
    ...
    the JSON envelope) and an optional per-line JS callback (live UI streaming).
...
def run_script(filename: str, source: str, extras: str = "") -> str:
    ...
    try:
        ...
        with contextlib.redirect_stdout(stdout_stream), contextlib.redirect_stderr(stderr_stream):
            ...
            try:
                # dont_inherit=True so this module's `from __future__ import annotations` does not leak into the
                # user's code: under PEP 563 the kernel's parameter annotations would stringize (`theta: float` ->
                # "float"), and holoso resolves scalar types by identity on the real annotation object.
                code = compile(user_file.read_text(encoding="utf-8"), str(user_file), "exec", dont_inherit=True)
